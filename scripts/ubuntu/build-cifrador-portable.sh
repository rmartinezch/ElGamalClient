...
JAR_PATH="$PROJECT_ROOT/target/ElGamalCipher-${APP_VERSION}.jar"
LAUNCHER_PATH="$IMAGE_ROOT/${APP_NAME}"

if [[ ! -f "$JAR_PATH" ]]; then
  echo "No se encontró el JAR esperado: $JAR_PATH" >&2
  exit 1
fi
...
cp -L "$PROJECT_ROOT/libs/libvecj-2.2.0.so" "$NATIVE_ROOT/"
cp -L "$PROJECT_ROOT/libs/libvmgj-1.3.0.so" "$NATIVE_ROOT/"

mapfile -t transitive_deps < <(
  {
    ldd "$PROJECT_ROOT/libs/libvecj-2.2.0.so" || true
    ldd "$PROJECT_ROOT/libs/libvmgj-1.3.0.so" || true
