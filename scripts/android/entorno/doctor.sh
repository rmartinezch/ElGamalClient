...
if [[ ! -x "$ANDROID_DIR/gradlew" ]]; then
  echo "[android-doctor] ERROR: no se encontró gradle wrapper en $ANDROID_DIR." >&2
  exit 1
fi
...
if [[ -f "$ANDROID_DIR/local.properties" ]]; then
  ...
  echo "[android-doctor] WARN: falta $ANDROID_DIR/local.properties (usar local.properties.example)."
fi
