...
class AndroidCipherRunner(private val context: Context) {
    ...
    fun importPublicKeyBytes(bytes: ByteArray): ImportedInput {
        val inputDir = File(context.filesDir, INPUT_DIR_NAME)
        val targetFile = File(inputDir, PUBLIC_KEY_FILE_NAME)
        inputDir.mkdirs()
        targetFile.writeBytes(bytes)
        return ImportedInput(
            targetFile = targetFile,
            byteCount = targetFile.length(),
            message = "publicKey escrita en ${targetFile.absolutePath}"
        )
    }

    fun importVotesText(text: String): ImportedInput {
        val inputDir = File(context.filesDir, INPUT_DIR_NAME)
        val targetFile = File(inputDir, VOTES_FILE_NAME)
        inputDir.mkdirs()
        targetFile.writeText(text, StandardCharsets.UTF_8)
        return ImportedInput(
            targetFile = targetFile,
            byteCount = targetFile.length(),
            message = "votos escritos en ${targetFile.absolutePath}"
        )
    }
    ...
    fun currentCiphertextsFile(): File {
        return File(File(context.filesDir, EXPORT_DIR_NAME), CIPHERTEXTS_FILE_NAME)
    }
