...
/**
 * TrueRNG source for macOS backed by the device USB serial interface.
 * On macOS the TrueRNG appears as {@code /dev/cu.usbmodemXXXXX}.
 */
public final class MacTrueRngRandomSource extends RandomSource {
    ...
    private MacTrueRngRandomSource(SerialPort serialPort) {
    ...
    public static Optional<RandomSource> tryCreate(Logger logger) {
        ...
        for (SerialPort candidate : SerialPort.getCommPorts()) {
            ...
            Optional<MacTrueRngRandomSource> source = openPort(candidate, logger);
            if (source.isPresent()) {
                return source.map(RandomSource.class::cast);
            }
        }

        logger.warning(() -> "macOS -hw: no se detectó un TrueRNG accesible por puerto serie. "
                + "Configura -Delgamal.rng.device=/dev/cu.usbmodemXXXX o ELGAMAL_RNG_DEVICE=/dev/cu.usbmodemXXXX.");
    ...
    public String humanDescription(boolean verbose) {
        ...
        return "MacTrueRngRandomSource(" + portName + suffix + ")";
    }
    ...
    static String normalizeConfiguredPortName(String rawValue) {
        ...
        // On macOS, strip /dev/ prefix since jSerialComm uses the bare name (e.g. cu.usbmodem11101)
    ...
    private static Optional<MacTrueRngRandomSource> openConfiguredPort(String configuredPort, Logger logger) {
    ...
    private static Optional<MacTrueRngRandomSource> openPort(SerialPort port, Logger logger) {
        ...
        if (!port.openPort()) {
            logger.warning(() -> String.format("macOS -hw: no se pudo abrir el puerto %s", portName));
            return Optional.empty();
        }

        try {
            MacTrueRngRandomSource source = new MacTrueRngRandomSource(port);
            source.inputStream = port.getInputStream();
            logger.info(() -> String.format("macOS -hw: usando TrueRNG en %s (%s)",
                    portName,
                    buildDescription(port)));
            return Optional.of(source);
        } catch (Exception e) {
            closePortQuietly(port);
            logger.warning(() -> String.format("macOS -hw: no se pudo inicializar el puerto %s: %s",
                    portName,
                    e.getMessage()));
            return Optional.empty();
        }
